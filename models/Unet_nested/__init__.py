...
from models.Unet_nested.layers import unetConv2, unetUp
from models.Unet_nested.utils import init_weights, count_param

class UNet_Nested(nn.Module):

    def __init__(self, in_channels=1, num_classes=2, feature_scale=2, is_deconv=True, is_batchnorm=True, is_ds=True):
    ...
    def forward(self, inputs):
        ...
        maxpool0 = self.maxpool1(X_00)    # 16*256*256
        X_10= self.conv10(maxpool0)      # 32*256*256
        maxpool1 = self.maxpool2(X_10)    # 32*128*128
        X_20 = self.conv20(maxpool1)     # 64*128*128
        maxpool2 = self.maxpool3(X_20)    # 64*64*64
        X_30 = self.conv30(maxpool2)     # 128*64*64
        maxpool3 = self.maxpool4(X_30)    # 128*32*32
