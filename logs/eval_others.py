...
def eval_func(im1, im2):
    ...
    TN = np.logical_and((1.0 - im1), (1.0 - im2))
    FP = np.logical_and(im1, (1.0 - im2))
    FN = np.logical_and((1.0 - im1), im2)

    if im1.shape != im2.shape:
        raise ValueError("Shape mismatch: im1 and im2 must have the same shape.")

    dsc_ = (2.0 * TP.sum()) / (FP.sum() + FN.sum() + (2 * TP.sum()))
...
if __name__ == "__main__":
    ...
    for method, pref in zipped:
        ...
        for im_num in range(84):
            label1_orig = imread(
                os.path.join(
                    label1_folder_path, "{}_{}.png".format(im_num, label1_orig_pref)
                ),
                as_gray=True,
            )

            im_res = cv2.imread(
                os.path.join(folder_path, "{}_{}.png".format(im_num, pref))
            )[:, :, 0]
