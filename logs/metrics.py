...
def main(args):
    ...
    if args.print_vals == 'dsc_scores':
        ...
        print('{:<25}  {:<25}  {:<25}  {:<25}  {:<25}'.format('model name', 'TP', 'TN', 'FP', 'FN'))
    elif args.print_vals == 'metrics':
        print('{:<25}  {:<10}  {:<10}  {:<10}  {:<10}  {:<10}'.format('model name', 'DSC', 'SE', 'SP', 'PRE', 'ACC')) #'Ch1 DSC',
        print('-'*75)
    ...
    for i in range(len(yaml_files)):
        ...
        with open(yaml_files[i]) as f:
            ...
            if args.print_vals == 'dsc_scores':
                print('{:<25}  {:<20}  {:<10}'.format(net_name, scores['mean_DSC']*100, scores['mean_dsc_loss']*100))
        ...
        overall_TP, overall_TN, overall_FP, overall_FN = [float(t) for t in (mask0+mask1+mask2)/84]

        coeff = {
            'm0': {
                'TP': m0_TP, 'TN': m0_TN, 'FP': m0_FP, 'FN': m0_FN
            },
            'm1': {
                'TP': m1_TP, 'TN': m1_TN, 'FP': m1_FP, 'FN': m1_FN
            },
            'm2': {
                'TP': m2_TP, 'TN': m2_TN, 'FP': m2_FP, 'FN': m2_FN
            },
            'overall': {
        ...
        if args.print_vals == 'evals':
            print('-'*125)
            print('{:<25}  {:<25}  {:<25}  {:<25}  {:<25}'.format(net_name, coeff['m0']['TP'], coeff['m0']['TN'], coeff['m0']['FP'], coeff['m0']['FN']))
            print('{:<25}  {:<25}  {:<25}  {:<25}  {:<25}'.format('', coeff['m1']['TP'], coeff['m1']['TN'], coeff['m1']['FP'], coeff['m1']['FN']))
            print('{:<25}  {:<25}  {:<25}  {:<25}  {:<25}'.format('', coeff['m2']['TP'], coeff['m2']['TN'], coeff['m2']['FP'], coeff['m2']['FN']))
            print('{:<25}  {:<25}  {:<25}  {:<25}  {:<25}'.format('', coeff['overall']['TP'], coeff['overall']['TN'], coeff['overall']['FP'], coeff['overall']['FN']))
        ...
        DSC = (2*TP)/(FP+FN+(2*TP))*100 if TP!=0 else TN/(TN+FP+FN+TP)*100
        SE = TP/(TP+FN)*100
        SP = TN/(TN+FP)*100
        PRE = TP/(TP+FP)*100
