...
def main():
    ...
    for i in range(len(grid_search)):
        ...
        axes[0].set_yticks([np.min(grid_y_axis), np.median(grid_y_axis), np.max(grid_y_axis)])
        ...
        axes[1].set_yticks([np.min(short_y_axis), np.median(short_y_axis), np.max(short_y_axis)])
