...
def dsc_score(im1, im2):
    ...
    TN = np.logical_and((1.-im1),  (1.-im2)).sum()
    FP = np.logical_and(im1, (1.-im2)).sum()
    FN = np.logical_and((1.-im1), im2).sum()

    if im1.shape != im2.shape:
        raise ValueError("Shape mismatch: im1 and im2 must have the same shape.")

    dsc_ = (2. * TP) / (FP + FN + (2 * TP))
...
if __name__ == '__main__':
    ...
    #im_origs = []
    for im_num in range(84):
        y_trues.append(cv2.imread(os.path.join(orig_path,"{}_label1_orig.png".format(im_num)))[:,:,0])
    ...
    for model in models:
        ...
        y_pred = []
        for im_num in range(84):
            y_pred.append(cv2.imread(os.path.join(pred_path,"{}_label1.png".format(im_num+1)))[:,:,0])
    ...
    t_vals_search = find_threshold_dsc(models, y_preds, y_trues, t_vals, print_vals=True, prefix="grid_search")

    t_vals = {}
    for m in models:
        t_vals[m] = list(range(t_vals_search[m]-t_step+1, t_vals_search[m]+t_step))
    t_vals_optimum = find_threshold_dsc(models, y_preds, y_trues, t_vals, print_vals=True, prefix="search_best")
